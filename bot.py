...
# 📊 STATISTICS COUNTERS (সারাদিনের হিসাব রাখার গ্লোবাল ভেরিয়েবল)
stats = {
    "MAIN": {"signals": 0, "wins": 0, "losses": 0},
    "VIP": {"signals": 0, "wins": 0, "losses": 0}
}
...
async def main_automated_loop():
    global pending_results, next_main_signal_time, next_vip_signal_time, stats, last_report_date
    pairs_list = list(REAL_FOREX_PAIRS.keys())
    print("🚀 TradeVision M5 High-Accuracy Engine v24.5 with Stats is Active...")
    ...
    while True:
        try:
            ...
            # 🕒 রাত ১১:৫৯ মিনিটে অটোমেটিক প্রতিদিনের পরিসংখ্যান রিপোর্ট পাঠানো এবং রিসেট লজিক
...
🎯 Powered by TradeVision AI Quant Engine"""
                    try:
                        ...
                        print(f"Failed to send stats report to {ch_type}: {ex}")
                
                # নতুন দিনের জন্য ডেটা রিসেট
                stats = {
                    "MAIN": {"signals": 0, "wins": 0, "losses": 0},
                    "VIP": {"signals": 0, "wins": 0, "losses": 0}
                }
...
⚠️ **WARNING :** Do NOT trade on OTC Market! Use Real Market only."""
                    ...
                    stats["MAIN"]["signals"] += 1 # সিগন্যাল কাউন্ট বৃদ্ধি
...
⚠️ **WARNING :** Use only on Real Market Quotex/PocketOption."""
                    ...
                    stats["VIP"]["signals"] += 1 # সিগন্যাল কাউন্ট বৃদ্ধি
                    pending_results.append({
                        "channel": "VIP", "pair": selected_pair, "ticker": ticker, 
                        "signal": signal, "entry_time": run_time, "expiry_time": expiry_t, "is_martingale": False
                    })

            # 🎯 ৩. ৫-মিনিট রেজাল্ট চেকার ও স্ট্যাটস আপডেট
            still_pending = []
            for item in pending_results:
                if now_bd >= (item["expiry_time"] + timedelta(seconds=8)):
                    ...
                    if result == "WIN":
                        ...
                        stats[ch_type]["wins"] += 1 # উইন কাউন্ট বৃদ্ধি
                    else:  
                        if not item["is_martingale"]:
                            ...
                            stats[ch_type]["losses"] += 1 # মার্টিনগেলও মিস হলে ফাইনাল লস কাউন্ট বৃদ্ধি
                else:
...
# ==================== KEEP ALIVE ====================
@app.route('/')
def home(): return "TradeVision M5 Stats Engine v24.5 is Online!"
